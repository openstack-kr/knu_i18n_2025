...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--repo-dir",
        required=True,
        help="Path to cloned repo. modulename을 여기서 조회")
    parser.add_argument(
        "--lang",
        required=True,
        help="Language code to translate (e.g., ko_KR, ja)")
    ...
    target_file_name = get_modulename(args.repo_dir, args.project)
    POT_FILE = os.path.join(POT_DIR, f"{target_file_name}.pot")

    # -----------------------------
    # languages Config
    # -----------------------------
    LANGUAGES_TO_TRANSLATE = [args.lang]

    # -----------------------------
    # LLM Config (hardcoded for CI)
    # -----------------------------
